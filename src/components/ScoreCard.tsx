      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="flex items-baseline gap-3 mt-1">
            <span className={`text-5xl font-bold ${scoreColor(result.total)}`}>{result.total}</span>
            <span className="text-ink-faint text-lg">/100</span>
            <span className="text-sm font-medium text-ink-mid">{scoreLabel(result.total)}</span>
          </div>
          {prev && delta !== 0 && (

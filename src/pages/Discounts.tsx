...
const NAV_SECTIONS = [
  { id: 'aggregators', label: 'Platforms' },
  { id: 'categories', label: 'By category' },
  { id: 'strategy', label: 'How to maximize' },
]
...
export default function Discounts() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-2xl font-bold text-ink">Student discounts</h1>
        <p className="text-sm text-ink-faint">
          Your .edu email is worth real money. Set these up once and save every month, no budgeting
          willpower required.
        </p>
      </header>

      {!searching && <PageNav sections={NAV_SECTIONS} />}
      ...
      <section id="aggregators" className="scroll-mt-6">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-ink-faint">Start with these three platforms</h2>
        <p className="mt-0.5 text-xs text-ink-faint">Set these up first. A single verification unlocks hundreds of brands at once.</p>
      ...
      {/* Discount groups */}
      <div id="categories" className="scroll-mt-6 space-y-5">
      {matchedGroups.map((group) => (
        <section key={group.category}>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-ink-faint">{group.category}</h2>
          <p className="mt-0.5 text-xs text-ink-faint">{group.blurb}</p>
          <div className="mt-2 grid sm:grid-cols-2 gap-3">
            {group.items.map((d) => (
              <div key={d.name} className="bg-surface rounded-xl border border-line p-4 flex gap-3 items-start hover:border-brand transition-colors">
      ...
      <section id="strategy" className="bg-surface rounded-2xl border border-line p-6 scroll-mt-6">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-ink-faint">How to actually get the most out of these</h2>
        <p className="mt-1 text-xs text-ink-faint">Six strategies that separate students who save a few dollars from students who save hundreds per year.</p>
      ...
      <p className="text-xs text-ink-faint">
        Prices and offers change. Always check the current terms before subscribing, and re-verify
        each school year where required.
      </p>
    </div>
  )
}
